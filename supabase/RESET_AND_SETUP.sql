-- ============================================================================
-- AdProof — CLEAN REBUILD. Use this when SETUP_ALL keeps failing on a stale
-- leftover table (e.g. "column user_id does not exist"). Paste this ENTIRE
-- file into the Supabase SQL editor of your app's project
-- (clypkvuhwfsahofaqgel) and Run ONCE.
--
-- WHAT IT DOES: drops and recreates the `public` schema — i.e. wipes every
-- app data table and starts fresh, so no stale table can block setup — then
-- runs the full schema and creates the storage bucket.
--
-- WHAT IT DOES NOT TOUCH: your login accounts (auth.users) and any uploaded
-- files (storage) live in SEPARATE schemas and are left completely alone.
--
-- SAFETY CHECK (optional): before running this, run just the next line in a
-- scratch query. If it errors or returns 0, there's nothing to lose:
--     select count(*) from public.companies;
-- ============================================================================

-- [reset] wipe the public schema and restore default Supabase grants ----------
...
-- AdProof — full database setup. Runs migrations 0001–0010 in order.
...
-- [bucket] creative uploads (private; app serves signed URLs) ------------------
